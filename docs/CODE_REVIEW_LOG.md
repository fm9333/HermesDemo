...
提交记录：
```text
ba6a0b9 stage 11 personal skill drafts v1
```

## 2026-05-18 阶段 12 Skill Patch 与回滚 v1 评审

范围：
```text
personal_skill_patches schema
PersonalSkillService patch lifecycle
Personal Skill rollback
Skill Patch API
客户端补丁面板
服务层与 API 测试
```
...
结论：
```text
通过，形成可提交点 stage-12-skill-patches-v1。
```
...
评审结论：
```text
Skill Patch v1 建立了个人技能的版本迭代闭环。
补丁必须声明原因、目标版本、候选 Prompt 和输出契约，并通过评测后才能应用。
评测会拦截版本过期补丁，避免基于旧版本覆盖新版本。
应用补丁会增加 Personal Skill 版本号，并写入版本快照。
回滚不会删除历史版本，而是将上一版本内容作为新的 rollback 版本写入，满足可审计要求。
客户端新增补丁面板和个人技能回滚入口。
Skill Curator、自动补丁生成和价值/成功率治理仍是后续 P0。
```
...
提交记录：
```text
b3bb8f0 stage 16 llm secret vault v1
```

## 2026-05-19 阶段 17 旧数据库启动兼容修复评审
...
结论：
```text
通过，形成可提交点 stage-17-legacy-database-migration-guard。
```
...
python -m compileall hermes_app tests
node --check hermes_app\web\static\app.js
python -m pytest tests\test_database_migrations.py tests\test_performance_indexes.py -q
python -c "from hermes_app.main import app; print('import ok')"
python -m pytest -q
```

评审结论：
```text
启动失败根因是旧 SQLite 表结构缺少 status 等后续版本新增列，而 Database.init 在补列之前先创建了依赖 status 的性能索引。
修复后初始化顺序改为：建表骨架 -> 补旧表缺失列 -> 创建索引 -> 记录迁移。
新增旧库回归测试会构造无 status 的 idea_cards、wardrobe_items、prd_drafts 表，确保升级启动不再触发 no such column: status。
当前默认开发库已通过 hermes_app.main 导入验证。
```
